...
public class SecurityConfig extends WebSecurityConfigurerAdapter  {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests() // HttpServletRequest에 따라 접근(access)을 제한
			...
			.antMatchers("/**","/fragments/**","/center/centerlist","/exerciseinfoboard/boardlist","/dietdiaryboard/boardlist","/healthboard/boardlist","/sharingboard/boardlist","/naversearch/**", "/upload/**", "/body/**").permitAll() //   누구나 접근 허용
...
//			.anyRequest().authenticated() // 나머지 요청들은 권한의 종류에 상관 없이 권한이 있어야 접근 가능
		.and()
			...
			.loginPage("/") // 로그인 페이지 링크 .... post의 이름이 같다면 loginProcessingUrl생략 	
